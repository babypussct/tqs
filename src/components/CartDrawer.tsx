...
export default function CartDrawer({ isOpen, onClose, items, onUpdateQuantity, onRemove }: CartDrawerProps) {
  ...
  const progress = Math.min((totalPrice / FREESHIP_THRESHOLD) * 100, 100);
  const remainingForFreeship = Math.max(FREESHIP_THRESHOLD - totalPrice, 0);
        ...
        <div className="p-6 bg-white dark:bg-zinc-950 border-b border-gray-100 dark:border-zinc-800">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-sm font-medium text-gray-600 dark:text-zinc-300">
              {progress === 100 ? (
                <span className="text-emerald-600 dark:text-emerald-500 font-bold">Chúc mừng! Bạn đã được Freeship.</span>
              ) : (
                <>Mua thêm <strong className="text-red-600 dark:text-red-500">{formatPrice(remainingForFreeship)}</strong> nữa để được Freeship</>
              )}
            </p>
          </div>
          <div className="h-2 w-full bg-gray-100 dark:bg-zinc-800 rounded-full overflow-hidden">
            <div 
              className={`h-full transition-all duration-500 rounded-full ${progress === 100 ? 'bg-emerald-500' : 'bg-red-600'}`}
          ...
          {items.length === 0 ? (
            ...
            items.map((item) => {
              ...
              <div key={item.id} className="flex gap-4 bg-white dark:bg-zinc-900 p-3 rounded-xl border border-gray-100 dark:border-zinc-800 shadow-sm">
                <div className="w-20 h-20 rounded-lg overflow-hidden bg-gray-50 dark:bg-zinc-800 shrink-0 border border-gray-100 dark:border-zinc-700">
                  {item.product.image ? (
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="text-gray-900 dark:text-white font-medium text-sm line-clamp-2 leading-snug mb-1">
                      {item.product.name}
                    </h3>
